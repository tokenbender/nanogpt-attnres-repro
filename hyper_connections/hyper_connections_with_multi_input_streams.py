...
class ProjActScale(Module):
    def __init__(
        ...
        self.maybe_squeeze = (
            Rearrange("... 1 -> ...") if squeeze_output else nn.Identity()
        )
...
class Residual(Module):
    ...
    def forward(self, residuals, *branch_args, **branch_kwargs):
        ...
        branch_input, residuals, residual_kwargs = self.width_connection(
            residuals, *branch_args, **branch_kwargs
        )
...
InputPathType = Union[
    int, str
]  # the path to the second residual stream, where `int` points to *args[`int`] and `str` points to **kwargs[`str`] - `int` needs to be > 0, as 0 is the default input residual stream
...
class HyperConnections(Module):
    def __init__(
        ...
        *,
        ...
        # activation used for the dynamic projections

        self.norm = RMSNorm(dim)  # uses RMSNorm instead of the paper's LayerNorm
        ...
        self.dynamic_alpha_and_branch_input = ProjActScale(
            dim, num_residual_streams + 1, activation=act
        )
        self.static_alpha = nn.Parameter(
            torch.cat([init_alpha0, torch.eye(num_residual_streams)], dim=1)
        )
        ...
        additional_input_paths = [
            one_path if isinstance(one_path, tuple) else (one_path, dim)
            for one_path in additional_input_paths
        ]

        assert all(
            [isinstance(path, str) or path > 0 for (path, _) in additional_input_paths]
        )

        self.additional_norms = ModuleList(
            [RMSNorm(dim) for _, dim in additional_input_paths]
        )
        self.additional_to_dynamic_input = ModuleList(
            [
                ProjActScale(dim, 1, activation=act, squeeze_output=True)
                for _, dim in additional_input_paths
            ]
        )
        self.additional_static_input = nn.ParameterList(
            [nn.Parameter(init_alpha0[..., 0]) for _ in additional_input_paths]
        )
    ...
    def width_connection(self, residuals, *branch_args, **branch_kwargs):
        ...
        residuals = rearrange(
            residuals, "(b s) ... d -> b ... s d", s=self.num_residual_streams
        )
        ...
        for (path, *_), norm, proj, learned_static in zip(
            self.additional_input_paths,
            self.additional_norms,
            self.additional_to_dynamic_input,
            self.additional_static_input,
        ):
            ...
            additional_residuals = rearrange(
            ...
            additional_residuals = einsum(
                additional_mix, additional_residuals, "... s, ... s d -> ... d"
            )
    ...
    def depth_connection(self, branch_output, residuals, *, beta):
        ...
        residuals = (
            einsum(branch_output, beta, "b ... d, b ... s -> b ... s d") + residuals
        )
    ...
    def decorate_branch(self, branch: Callable):
        ...
        def forward_and_add_residual(residual, *args, **kwargs):
            ([branch_input, *args], kwargs), add_residual = self.forward(
    ...
    def forward(self, residuals, *branch_args, **branch_kwargs):
        ...
        (branch_args, branch_kwargs), residuals, residual_kwargs = (
            self.width_connection(residuals, *branch_args, **branch_kwargs)
        )
