...
from hyper_connections.hyper_connections import Residual, RMSNorm
...
class HyperConnections(Module):
    def __init__(
        ...
        *,
        ...
        self.dynamic_beta_fn = nn.Sequential(
            nn.Conv2d(dim, 1, 1, bias=False), Rearrange("b 1 ... -> b ...")
        )
    ...
    def width_connection(self, residuals):
        ...
        dynamic_alpha = rearrange(
            dynamic_alpha, "(b s) ... -> b s ...", s=self.num_residual_streams
        )
        ...
        dynamic_beta = rearrange(
            dynamic_beta, "(b s) ... -> b s ...", s=self.num_residual_streams
        )
        beta = dynamic_beta + rearrange(self.static_beta, "s -> s 1 1")

        residuals = rearrange(
            residuals, "(b s) ... -> b s ...", s=self.num_residual_streams
        )
